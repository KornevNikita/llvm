      ...
      if (!isComposite) {
        ...
        if (DeviceDefaultGPU) {
          ...
          ur_result_t UrRet = ur::level_zero::urDeviceGetInfo(
              common_cast(D.get()), UR_DEVICE_INFO_MAX_COMPUTE_UNITS,
...
ur_result_t urDeviceGetInfo(
  ...
  // TODO: Level Zero does not report the IGCA of the device yet. Return dummy
  // values until an extension to zeDeviceGetProperties exposes it. 0 is not a
  // valid IGCA, so it can't be mistaken for a real one.
  case UR_DEVICE_INFO_IGCA:
    return ReturnValue(uint32_t{0});
  case UR_DEVICE_INFO_IGCA_FEATURE_SET:
    return ReturnValue(UR_DEVICE_IGCA_FEATURE_SET_RENDER);
  default:
    UR_LOG(ERR, "Unsupported ParamName in urGetDeviceInfo");
...
ur_result_t urDevicePartition(
    ...
    ::ur_device_handle_t *OutDevicesOpque,
    /// [out][optional] pointer to the number of sub-devices the device can be
    /// partitioned into according to the partitioning property.
    uint32_t *NumDevicesRet) {
  auto Device = common_cast(hDeviceOpque);
  ur_device_handle_t *OutDevices = common_cast(OutDevicesOpque);
  ...
  for (uint32_t I = 0; I < NumDevices; I++) {
    ...
    ur::level_zero::urDeviceRetain(common_cast(OutDevices[I]));
  }
...
ur_result_t urDeviceSelectBinary(
    ...
    [[maybe_unused]] ::ur_device_handle_t DeviceOpque,
...
ur_result_t urDeviceGetNativeHandle(
  ...
  *NativeDevice =
      reinterpret_cast<ur_native_handle_t>(common_cast(DeviceOpque)->ZeDevice);
  return UR_RESULT_SUCCESS;
}
...
ur_result_t urDeviceCreateWithNativeHandle(
    ...
    ::ur_native_handle_t NativeDevice,
    /// [in] handle of the platform instance
    ::ur_adapter_handle_t AdapterOpque,
...
ur_result_t urDeviceWaitExp(::ur_device_handle_t hDeviceOpque) {
  auto hDevice = common_cast(hDeviceOpque);
  ZE2UR_CALL(zeDeviceSynchronize, (hDevice->ZeDevice));
  return UR_RESULT_SUCCESS;
}
...
bool ur_context_common_t::isValidDevice(ur_device_handle_t hDevice) const {
  while (hDevice) {
